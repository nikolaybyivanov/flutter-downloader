...
import 'package:test/player_screen.dart';
import 'package:test/video_player_mode.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<String> getTaskLocalPath(_TaskInfo _taskInfo) async {
    final localPath = (await _findLocalPath())!;
    if (_taskInfo.status == DownloadTaskStatus.complete) {
      final taskName = _taskInfo.link!.split('/').last;
      return localPath + '/' + taskName;
    }
              ...
              .map((item) => item.task == null
                  ...
                  : DownloadItem(
                      ...
                      onItemClick: (task) async {
                        ...
                        final mode = task.status == DownloadTaskStatus.complete
                            ? VideoPlayerMode.offline
                            : VideoPlayerMode.online;

                        final path = task.status == DownloadTaskStatus.complete
                            ? (await getTaskLocalPath(task))
                            : task.link;
  ...
  Future<bool> _checkPermission() async {
    ...
    if (widget.platform == TargetPlatform.android &&
        (await deviceInfo.androidInfo).version.sdkInt <= 28) {
